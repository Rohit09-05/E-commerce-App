...
// Delete confirmation inline modal component
...
function AdminPage() {
  ...
  const confirmDelete = async () => {
    ...
    setDeleteError("");
    try {
      await axios.delete(`/api/products/${productToDelete._id || productToDelete.id}`);
      setProductToDelete(null);
  ...
  const toggleOrderStatus = async (orderId) => {
    try {
      ...
      await axios.put(`/api/orders/${orderId}`, { fulfilled: updatedStatus });
      await refreshOrders();
    } catch (err) {
  ...
  {products.map(product => (
    <li
      ...
      // Added rounded-2xl, shadow, transition, flex on li level for smoother look
    >
      <img
